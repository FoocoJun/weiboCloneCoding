...
const CenterFeedSection = () => {
  ...
  const observerBoxRef = React.useRef();
  let filteredPost = tmpPost.filter((val, idx) => idx < 5 * showMore);
  const toShowMore = () => {
    if (5 * (showMore + 1) > tmpPost.length) {
      setShowMore(showMore + 1);
      observerBoxRef.current.style.display = "none";
      return;
    }
    // observerBoxRef.current.style.display="none"
    //showMore이 이벤트 형성시 상수로 들어가서 안바뀜.
    setShowMore(showMore + 1);
    console.log("파바바바밧");
    //showMore 증가 후 이벤트 삭제
    observerBoxRef.current.removeEventListener("mouseover", toShowMore);
    observerBoxRef.current.removeEventListener("touchstart", toShowMore, {
      passive: true,
    });
    console.log("이벤트 삭제");
  };
  ...
  React.useEffect(() => {
    //showMore 증가에 따른 이벤트 삭제 후 업데이트 하여 이벤트 재생성
    observerBoxRef.current.addEventListener("mouseover", toShowMore);
    observerBoxRef.current.addEventListener("touchstart", toShowMore, {
      passive: true,
    });
    console.log("이벤트 생성");
  }, [showMore]);
